...
| `ANALYTICS_ENABLED` | `true` | Master toggle for analytics tracking; set to `false` for strict no-tracking runtime behavior |
...
- Analytics provider credentials and endpoints are configured in `config/services.php` and are not end-user configurable at runtime.
...
- `auth.register`: registration outcome (`status`, `requires_approval`, `requires_verification`)
- `auth.verify_email`: email verification outcome (`status`)
- `auth.invite_accept`: invitation acceptance outcome (`status`)
- `auth.2fa_enabled`: two-factor enrollment completion (`method`, `backup_codes_issued`)
- `auth.2fa_disabled`: two-factor disable completion (`method`, `revoke_app_passwords`)
- `auth.2fa_challenge_failed`: failed two-factor login challenge (`method`, `status`)
- `admin.setting_changed`: admin setting mutation (`setting_key`, `from_state`, `to_state`)
